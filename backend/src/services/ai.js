...
async function generateMessage(shopId, type, context) {
  ...
  const prompts = {
    ...
    reserva_recibida: `${TONE}\n\nAvisale a ${context.clientName} que tu reserva en ${context.shopName} fue recibida para el ${context.fecha} a las ${context.hora}${context.serviceName ? ` (${context.serviceName})` : ''}. Cuando el barbero la confirme te avisamos. Máximo 3 líneas.`,
